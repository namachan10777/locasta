...
#slide[
  ...
  #speaker-note[
    背景として、HPCではアクセラレータ中心のFat-node構成が主流になっています。
    ノード数自体は減少傾向ですが、ノード内のプロセス数はむしろ増加しており、
    富岳NEXTでは100万オーダーのプロセス数が想定されています。
    このため、ミドルウェアにはプロセス数に対するスケーラビリティが必要であり、
    処理性能の向上に伴ってよりスケーラブルなI/Oが常に求められています。
  ]
]
...
#slide[
  ...
  #speaker-note[
    ...
    IPC領域は要求キューと応答スロットで構成されます。
    ローカル完結する要求は専用のIPCキューで処理し、リモート経路と分離しています。
    デーモンはペイロードの内容を解釈せず、境界管理とWQE発行に専念する設計です。
  ]
]
